...
{
    template<typename T>
    Matrix<T> make_banded(const std::vector<T>& diag, const std::vector<T>& upper, const std::vector<T>& lower)
    {
    ...
    void make_triangular(std::size_t rows, std::size_t cols)
    {
        Matrix<T> mat(rows, cols);
        mat.template to_triangular<TT>();

        return mat;
    }

    template<typename T>
    Matrix<T> make_symmetric(const std::vector<T>& diag, const std::vector<T>& triangle)
    {        
    ...
    template<typename T>
    Matrix<T> make_toeplitz(const std::vector<T>& vector)
    {
    ...
    template<typename T>
    Matrix<T> make_vandermonde(const std::vector<T>& vec, double max_exponent)
    {
